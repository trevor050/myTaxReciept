...
import type { SuggestRepresentativesOutput } from '@/ai/flows/suggest-representatives'; // Use simplified output type
...
interface TaxBreakdownDashboardProps {
  ...
  representativeSuggestion: SuggestRepresentativesOutput | null; // Use simplified output type
}
...
const CustomTooltip = ({ active, payload, label, totalAmount }: any) => { // Added totalAmount prop
  if (active && payload && payload.length) {
    ...
    return (
        ...
        <div className="rounded-lg border bg-popover p-2.5 text-popover-foreground shadow-lg animate-fadeIn text-xs max-w-[150px] sm:max-w-[200px]"> {/* Added max-width */}
             <div className="flex items-center justify-between mb-1 gap-2">
                 <span className="font-medium flex items-center gap-1.5 truncate"> {/* Allow truncation */}
                    <Icon className="h-3 w-3 text-muted-foreground shrink-0" /> {/* Prevent icon shrink */}
                    {data.category}
                 </span>
...
const CustomLegend = (props: any) => {
  ...
  return (
    // Use flex-wrap for better wrapping on smaller screens
    <ul className="flex flex-wrap justify-center gap-x-3 gap-y-1.5 text-xs mt-4 list-none p-0 max-w-full mx-auto">
      {payload.map((entry: any, index: number) => {
          ...
          // const Icon = categoryIcons[entry.value] || DefaultIcon; // Icon can be noisy here
          return (
            <li key={`item-${index}`} className="flex items-center space-x-1 cursor-pointer text-muted-foreground hover:text-foreground transition-colors">
              ...
              <span className="truncate max-w-[100px] sm:max-w-[150px]">{entry.value}</span> {/* Adjust max-width */}
...
export default function TaxBreakdownDashboard({
  ...
  const handleOpenModal = () => {
    if (selectedItems.size > 0) {
      setIsModalOpen(true);
        ...
        // Show a toast if no items are selected
         toast({
  ...
  return (
    ...
    <div className="space-y-10 animate-fadeIn relative pb-28 sm:pb-24"> {/* Increased padding-bottom for floating button */}
        {/* --- Header --- */}
       ...
       {/* --- AI Suggestion / Action Prompt (Simplified) --- */}
        {representativeSuggestion && representativeSuggestion.topSpendingCategories.length > 0 && ( // Only show if suggestion exists and has categories
            <Alert className="mb-8 shadow-sm rounded-lg border border-primary/20 bg-primary/5 text-foreground">
                ...
                <AlertDescription className="text-sm text-foreground/90">
                    {/* Simplified reason based on top spending */}
                    Your breakdown highlights significant spending in: <span className="font-medium">{representativeSuggestion.topSpendingCategories.join(', ')}</span>.
                    These are common areas for public discussion.
                    <span className="block mt-1.5">Select specific sub-items below if you wish to contact your representative about potential overspending.</span>
      ...
      <div className="mb-12">
         ...
         <TooltipProvider delayDuration={150}> {/* Slightly longer delay */}
              <ResponsiveContainer width="100%" height={320}> {/* Adjusted height */}
                <PieChart margin={{ top: 5, right: 5, bottom: 5, left: 5 }}> {/* Reduced margins */}
                  <Pie
                    data={chartData} // Use prepared chartData
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    outerRadius={window.innerWidth < 640 ? 80 : 100} // Smaller radius on mobile
                    innerRadius={window.innerWidth < 640 ? 50 : 65} // Adjust inner radius too
                    fill="#8884d8"
                    paddingAngle={1} // Slightly reduced padding
                    dataKey="percentage"
                    nameKey="category"
                  >
                   ...
                   <Legend content={<CustomLegend />} wrapperStyle={{ maxWidth: '100%', overflow: 'hidden' }}/> {/* Ensure legend doesn't overflow */}
                </PieChart>
              </ResponsiveContainer>
            </TooltipProvider>
             <p className="text-xs text-muted-foreground text-center mt-4 flex items-center justify-center gap-1 px-2"> {/* Added horizontal padding */}
      ...
      {/* --- Detailed Breakdown Card --- */}
       <Card className="shadow-lg border border-border/60 rounded-xl overflow-hidden bg-gradient-to-b from-card to-card/95">
            <CardHeader className="px-4 py-4 sm:px-6 sm:py-5 border-b border-border/50"> {/* Adjusted padding */}
                <CardTitle className="text-lg sm:text-xl font-semibold tracking-tight">Detailed Spending</CardTitle> {/* Simplified title */}
            ...
            <CardContent className="p-0"> {/* Remove default padding */}
              <TooltipProvider delayDuration={250}> {/* Slightly longer tooltip delay */}
                    ...
                    {taxSpending.map((item, index) => {
                        ...
                        return (
                             <AccordionItem value={`item-${index}`} key={item.id || index} className="border-b border-border/40 last:border-b-0 group">
                                <AccordionTrigger className="hover:no-underline py-3 px-3 sm:px-4 rounded-none hover:bg-accent/50 data-[state=open]:bg-accent/40 transition-colors duration-150 text-left"> {/* Ensure text aligns left */}
                                     <div className="flex justify-between items-center w-full gap-2 sm:gap-3"> {/* Reduced gap */}
                                        <div className="flex items-center gap-2 min-w-0"> {/* Reduced gap */}
                                            <Icon className="h-4 w-4 text-primary shrink-0" />
                                            <span className="font-medium text-sm truncate flex-1">{item.category}</span> {/* Reduced text size */}
                                        </div>
                                        <div className="text-right shrink-0 flex items-baseline gap-1 ml-auto"> {/* Reduced gap */}
                                            <span className="font-semibold font-mono text-sm">${formatCurrency(categoryAmount)}</span> {/* Reduced text size */}
                                            <span className="text-muted-foreground text-xs font-mono hidden sm:inline">({item.percentage.toFixed(1)}%)</span> {/* Hide percentage on mobile */}
                                        </div>
                                ...
                                >
                                    <div className="pl-8 pr-3 sm:pl-10 sm:pr-4 pt-3 pb-4 text-muted-foreground space-y-2.5"> {/* Adjusted padding */}
                                     {isInterestOnDebt && (
                                    ...
                                    {hasSubItems ? (
                                        <ul className="space-y-2">
                                            {item.subItems!.map((subItem) => {
                                                ...
                                                return (
                                                     <li key={subItem.id} className="flex justify-between items-center text-xs gap-2 group/subitem"> {/* Reduced gap */}
                 ...
                 {/* --- Total Row --- */}
                 <div className="flex justify-between items-center w-full px-3 sm:px-4 py-3 sm:py-4 border-t-2 border-primary/50 bg-primary/5"> {/* Adjusted padding */}
       ...
       {/* Floating Action Button - Updated Styling */}
        {selectedItems.size > 0 && (
            <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 animate-slideInUp transition-all duration-300 ease-out"> {/* Adjusted bottom position */}
                <Button
                    size="lg" // Keep large size for prominence
                    className="shadow-2xl rounded-full text-sm sm:text-base px-5 py-3 sm:px-6 sm:py-3 bg-gradient-to-r from-primary to-teal-600 hover:from-primary/90 hover:to-teal-700 text-primary-foreground animate-glow flex items-center gap-2 ring-2 ring-primary/30 ring-offset-2 ring-offset-background" // Gradient, refined glow, ring
